...
Info buffer selection is 61=0; effect read selection 63=1; cursor reset 21=0; bytes read from 40 with checksum 42. A 20-byte effect is staged through 0B=0, 21=0, repeated 20 writes, checked against 42, then applied with 82=1. This firmware temporarily NACKs while applying; bounded status-30 polling handles that interval, and full effect readback must match before success. The original brightness-only OFF described by early tests was insufficient optically and is superseded by the correction below. Failure attempts restoration of both prior effects and reports restoration errors explicitly.

### 2026-09-13: physically confirmed RAM OFF

The user observed continued light despite matching mode 0x10 effect readback with brightness zero and, later, both custom RGB endpoints black. Mode 0x10 consumes the separate per-LED table, not these custom colors; see the primary [controller mode definitions](https://github.com/CalcProgrammer1/OpenRGB/blob/master/Controllers/CorsairDRAMController/RGBController_CorsairDRAM.cpp). No per-LED table write was added. The successful packet is `01 01 01 00 00 00 00 03 00 00 00 03 00 00 00 00 00 00 00 00`: both pulse endpoints are black, so there is no visible pulse. Both modules matched immediately and after releasing the bus for eight seconds, and the user confirmed visible OFF.

Power OFF and every lighting effect with requested brightness 0 now generate that exact packet. Session state remembers each DIMM's prior effect only after both writes succeed; repeated OFF preserves that memory. With no session history, ON falls back to the existing static LED table. The existing application-level lighting cache still takes priority on ON. `ram_off_probe` verifies repeated power OFF and all three zero-brightness paths on both DIMMs, then leaves OFF in place. Effect readbacks and remembered state are tested separately; we do not infer optical success solely from CRC/readback.
