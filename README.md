...
**전체 프로파일 → 새 프로파일**에서 이름을 입력하고 기존 장치 펼침 영역에서 5개 장치의 ON/OFF·색상·효과·밝기·속도를 편집합니다. 마우스의 **DPI · 표시등** 탭에서 DPI 설정 포함 여부도 정할 수 있습니다. 새 프로파일은 이번 실행의 최근 적용값을 사용하며, 기록이 없는 장치는 분홍색 단색 70%로 시작합니다. 취소하면 실제 장치 설정은 바뀌지 않습니다.

**저장·적용**은 프로파일을 파일에 먼저 저장하고 전체 장치에 바로 적용합니다. 저장 실패 시 장치에는 명령을 보내지 않습니다. 일부 장치가 실패하면 나머지 결과와 장치별 오류를 표시하며, 저장된 프로파일을 선택해 **일괄 적용**으로 다시 시도할 수 있습니다. **수정**은 선택한 프로파일의 이름과 장치 설정을 갱신합니다. 프로파일은 최대 32개이며 앱 설정 폴더의 `profiles.json`에 저장되어 재실행 후에도 유지됩니다. 로그인 시 프로파일을 자동 적용하지는 않습니다. 메인보드·팬과 메모리 두 개는 기존과 같이 묶음 제어합니다. 브라우저 데모 프로파일은 실제 앱과 분리해 저장합니다.
...
메모리 OFF는 두 모듈에서 실제 소등을 확인한 검정색 효과를 사용합니다. 기존 정적 모드에서 밝기만 0으로 바꾸는 방식은 설정 재조회가 성공해도 실제로 꺼지지 않았습니다. 전체·개별·프로파일 OFF와 밝기 0%에 같은 소등 처리를 적용합니다. OFF 전 메모리 효과는 이번 실행 동안 모듈별로 기억하고, 반복 OFF로 덮어쓰지 않습니다. ON은 최근 앱 조명 설정을 우선 사용하며, 기록이 없으면 OFF 전 효과 또는 기본 정적 모드로 돌아갑니다.

## 모니터 밝기

장치 목록 아래 **모니터 밝기**에서 연결된 화면별 밝기를 조절합니다. 지원 화면이 여러 대면 **전체 밝기 맞추기**로 함께 변경할 수 있습니다. 슬라이더를 놓거나 방향키를 누르면 바로 적용하고, 모니터에서 재조회한 값을 표시합니다. 전체 슬라이더의 초기값은 지원 화면 밝기의 평균입니다. 0%는 화면 전원 OFF가 아니라 해당 모니터의 최소 밝기입니다.

Windows 내장 DDC/CI API를 사용하므로 별도 프로그램이나 모니터 드라이버 파일을 배포하지 않습니다. 모니터 메뉴에서 DDC/CI가 켜져 있어야 합니다. HDR·자동 밝기·연결 어댑터에 따라 제어가 제한될 수 있고, 실패한 화면에는 오류가 표시됩니다. 케이블 연결이나 화면 배치 변경 후 **다시 검색**을 누르세요. RGB 전체 ON/OFF 및 RGB 프로파일은 모니터 밝기를 변경하지 않으며, 앱 시작 시 밝기를 자동 적용하지 않습니다. 노트북 내장 패널용 WMI 밝기 제어는 포함하지 않습니다.

이 PC의 LG ULTRAWIDE 2대에서 각각 100% → 99% → 100% 변경 및 재조회를 확인했습니다. 광학 센서로 측정한 밝기가 아닌 모니터가 보고한 설정값입니다. 진단은 `cargo run --manifest-path src-tauri/Cargo.toml --example monitor_probe`로 읽기만 수행하며, `-- --verify`를 붙이면 각 화면을 1% 변경한 뒤 원래 값으로 복원합니다.
